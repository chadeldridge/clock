...
func TestClockNewFromJSON(t *testing.T) {
	...
	t.Run("valid", func(t *testing.T) {
		c, err := NewClockFromJSON([]byte(`{"min":0,"max":4,"val":3,"inc":1,"orig":0}`))
	...
	t.Run("invalid", func(t *testing.T) {
		...
		_, err := NewClockFromJSON([]byte(`{"min":b,"max":4,"val":3,"inc":1,"orig":0}`))
	...
	t.Run("invalid min", func(t *testing.T) {
		_, err := NewClockFromJSON([]byte(`{"min":-4,"max":4,"val":3,"inc":1,"orig":0}`))
		require.Error(err, "NewClockFromJSON() did not return an error: %s", err)
		require.Equal("invalid Clock: min must be 0", err.Error())
	})

	t.Run("invalid inc", func(t *testing.T) {
		_, err := NewClockFromJSON([]byte(`{"min":0,"max":4,"val":3,"inc":2,"orig":0}`))
		require.Error(err, "NewClockFromJSON() did not return an error: %s", err)
